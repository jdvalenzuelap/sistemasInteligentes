...
class Node(object):
    ...
    def __init__(self, state: List[List[int]], parent, g_value: int, f_value: int, movement: str):
        self.state = state
        self.parent = parent
...
class Puzzle(object):
    ...
    def solve(self):
        ...
        start_node = Node(self.initial_state, None, self.INITIAL_G_VALUE,
                          self._find_minimum_heuristic_among_final_states(self.initial_state), 'Iniciamos')
        ...
        while open_list:
            ...
            if Puzzle._check_is_node_in_target_state_list(current_node.state, self.final_states):
                ...
                for child_state in expanded_children:
                    is_in_open_list, open_node = Puzzle._check_is_node_in_node_list(child_state['node'], open_list)
                    is_in_closed_list, closed_node = Puzzle._check_is_node_in_node_list(child_state['node'], closed_list)

                    if not is_in_open_list and not is_in_closed_list:
                        open_list.append(
                            Node(child_state['node'], current_node, current_g_value,
                                 self._find_minimum_heuristic_among_final_states(child_state['node']) + current_g_value,
                                 child_state['direction']
                                 )
                        )
                    else:
                        if is_in_open_list:
...
if __name__ == '__main__':
    ...
    if solution_exists:
        for node in solution_path:
            if node.movement != 'Iniciamos':
                movement_string += node.movement + ','
